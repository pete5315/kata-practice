...
export const demovingShift = (arr: string[], shift: number) => {
  let shiftedStringArray: string[] = unseparateIntoParts(arr).split("");
  console.log("shiftedString",shiftedStringArray);
  let outputString: string = "";
  for (let i = 0; i < shiftedStringArray.length; i++) {
...
function shiftCharByN(char: string, n: number): string {
  if (/^[A-Z]$/.test(char)) {
...
function separateIntoParts(message: string): string[] {
  let outputArray: string[] = [];
  let messageLength: number = Math.ceil(message.length / 5);
...
function unshiftCharByN(char: string, n: number): string {
  if (/^[A-Z]$/.test(char)) {
...
function unseparateIntoParts(messages: string[]): string {
  let outputString: string = "";
  for (let i = 0; i < 5; i++) {
