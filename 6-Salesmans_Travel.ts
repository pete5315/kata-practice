...
export const travel = (r: string, zipcode: string): string => {
  ...
  let outputString: string = zipcode
  let stringArray: string[] = r.split(',');
//   console.log(stringArray)

  function checkZip(arrayString: string): boolean {
    return arrayString.search(zipcode) >= 0
  };
  
  let confirmedAddresses: string[] = stringArray.filter(checkZip)
  outputString = confirmedAddresses[0].slice(confirmedAddresses[0].length-8) + ":";
  console.log("outputString check", outputString)
  let stateZip: string = outputString;
  for (let address of confirmedAddresses) {
