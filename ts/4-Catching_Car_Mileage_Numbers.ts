...
function checkIsInteresting(n: number, awesomePhrases: number[]): boolean {
...
function checkForZeroes(n: number): boolean {
  let bool = false
  while (n >= 10) {
console.log("n", n)
    n = n/10;
  }
  if (Math.floor(n) === n ) { bool = true; }
  return bool
}

function sameDigits(n: number): boolean {
...
function incrementingDigits(n: number): boolean {
  let bool = true;
...
function decrementingDigits(n: number): boolean {
  let bool = true;
  let nString = n.toString();
  let nStringArray = nString.split("");
  for (let i = 1; i < nStringArray.length; i++) {
...
function palindrome(n: number): boolean {
  let bool = false;
  let nString: string = n.toString();
  let nArray: string[] = nString.split('');
  let nArrayReverse: string[] = [];
  console.log(nString, nArray);
  for (let i = nArray.length - 1; i >= 0; i--) {
    nArrayReverse.push(nArray[i]);
  }
  let nStringReverse = nArrayReverse.join("");
...
function awesomePhrasesCheck(n: number, awesomePhrases: number[]): boolean {
  let bool = false;
  for (let phrase of awesomePhrases) {
    if (phrase === n) {
      bool = true;
      break;
    }
